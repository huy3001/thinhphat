...
<?php
	...
	if ($info_list_link_apply == 'show-button') {
		...
		if ($button_icon !== '' || $button_title !== '') {
			...
			if($icon_position == 'left') {
				$info_list_link_html = '<a class="shortcode_button btn_normal btn_type1 icon_pos_'.$icon_position.' '.$only_icon.'" href="'.$url.'" '.$link_for_button.' '.$button_target.'>' . $button_icon . '' . $button_title . '</a>';
			} else {
				$info_list_link_html = '<a class="shortcode_button btn_normal btn_type1 icon_pos_'.$icon_position.' '.$only_icon.'" href="'.$url.'" '.$link_for_button.' '.$button_target.'>' . $button_title . '' . $button_icon . '</a>';
			}
		} else {
			$info_list_link_html = '';
		}
	...
	if($content !== '') {	
		$compile .= '<div class="cont_info" style="color:' . $text_color . '">' . wpb_js_remove_wpautop($content, true) . '</div>';
	}
