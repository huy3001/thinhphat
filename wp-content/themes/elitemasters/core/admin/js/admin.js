...
jQuery(document).ready(function () {
    ...
    if (jQuery('.page_layout').val() == 'clean') {
        jQuery('.layout_boxed_options').slideUp(1);
    }
    if (jQuery('.page_layout').val() == 'default') {
        if (jQuery('.layout_boxed_options').hasClass('no_boxed')) {
            jQuery('.layout_boxed_options').slideUp(1);
        }
    }
    ...
    jQuery('.page_layout').change(function(){
        if (jQuery(this).val() == 'clean') {
            jQuery('.layout_boxed_options').stop().slideUp(300);
        } else {
            jQuery('.layout_boxed_options').stop().slideDown(300);
        }
        if (jQuery(this).val() == 'default') {
            if (jQuery('.layout_boxed_options').hasClass('no_boxed')) {
                jQuery('.layout_boxed_options').stop().slideUp(300);
...
jQuery(window).load(function () {
	...
	else if (nowpostformat == 'quote') {
		jQuery('#quote_sectionid_inner').show();
		jQuery('.wp-admin.post-type-post #pb_section').show();
	}
	else if (nowpostformat == 'link') {
	...
	jQuery('#post-formats-select input').on("click", function () {
		jQuery('#portslides_sectionid_inner, #audio_sectionid_inner, #video_sectionid_inner, #quote_sectionid_inner, #link_sectionid_inner, #default_sectionid_inner').hide();
		...
		else if (nowclickformat == 'quote') {
			jQuery('#quote_sectionid_inner').show();
			jQuery('.wp-admin.post-type-post #pb_section').show();
		}
		else if (nowclickformat == 'link') {
			jQuery('#link_sectionid_inner').show();
			jQuery('.wp-admin.post-type-post #pb_section').show();
		}
		else if (nowclickformat == 'audio') {
			jQuery('#audio_sectionid_inner').show();
			jQuery('.wp-admin.post-type-post #pb_section').show();
		}
